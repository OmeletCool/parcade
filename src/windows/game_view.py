...
class DemoGameView(arcade.View):
    ...
    def update_background_position_and_size(self):
        ...
        self.phone_base_sprite.scale = (
            win_h * 0.22) / self.phone_base_sprite.texture.height
        self.phone_base_sprite.center_x, self.phone_base_sprite.center_y = win_w * 0.45, win_h * 0.3

        self.phone_tube_sprite.scale = (
            win_h * 0.09) / self.phone_tube_sprite.texture.height
        ...
        self.door_sprite.scale = (win_h * 0.8) / \
            self.door_sprite.texture.height
    ...
    def on_update(self, delta_time):
        ...
        if self.intro_active:
            ...
            is_last_slide = (self.current_slide_idx ==
                             len(self.intro_textures) - 1)
            ...
            if self.slide_time < self.fade_duration:
                ...
                elapsed = self.slide_time - \
                    (self.slide_duration + self.fade_duration)
                self.fade_sprite.alpha = int(
                    255 * (elapsed / self.fade_duration))
            else:
                self.fade_sprite.alpha = 0
    ...
    def on_mouse_motion(self, x, y, dx, dy):
        ...
        for sprite in [self.bed_sprite, self.door_sprite]:
            if hasattr(sprite, 'hover_text') and sprite.hover_text:
                sprite.texture = sprite.hover_text if sprite.collides_with_point(
                    (x, y)) else sprite.normal_text
