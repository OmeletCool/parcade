...
class BackyardView(arcade.View):
    ...
    def setup(self):
        ...
        if self.is_night:
            bg_tex = self.reg.get(
                '1episode/textures/backgrounds/backjard_night.png')
            self.locker_texture = self.reg.get(
                '1episode/textures/ui/buttons/normal/locker_night.png')
            self.locker_texture_hovered = self.reg.get(
                '1episode/textures/ui/buttons/hovered/locker_night_hovered.png')
        else:
            bg_tex = self.reg.get(
                '1episode/textures/backgrounds/backjard_day.png')
            self.locker_texture = self.reg.get(
                '1episode/textures/ui/buttons/normal/locker_day.png')
            self.locker_texture_hovered = self.reg.get(
        ...
        if self.window.game_state.get("postman_in_backyard", False):
            if self.is_night:
                postman_tex = self.reg.get(
                    '1episode/textures/npc/postman_night.png')
            else:
                postman_tex = self.reg.get(
                    '1episode/textures/npc/postman_day.png')
    ...
    def on_mouse_press(self, x, y, button, modifiers):
        ...
        if self.postman_sprite and self.postman_sprite.collides_with_point((x, y)):
            self.dialog_box.start_dialogue([
                DialoguePhrase(LANGUAGES['dialogues']['postman_talkings']['6'][self.language],
                               voice=Voice.DEFAULT, logo=Icon.POSTMAN_SAD, speed=10, callback=self.on_postman_taking_photos)
            ])
            return
    ...
    def on_postman_taking_photos(self):
        self.window.game_state['postman_in_backyard'] = False
    ...
    def _handle_gate_click(self):
        if self.window.game_state["is_gate_locked"]:
            if "gate_key" in self.window.inventory:
                ...
                self.dialog_box.start_dialogue([
                    DialoguePhrase(
                        LANGUAGES['dialogues']['items']['gate_locked'][self.language], voice=Voice.DEFAULT)
                ])
            return
